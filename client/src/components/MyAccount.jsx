// MyAccount — booking + rating history, XP/level, avatar, password change.
// Calls the real, auth-protected /api/users/profile endpoint with the JWT
// from localStorage — the backend derives identity from the token itself,
// never from a client-supplied email, so this can only ever show the
...
// Must match server/utils/xp.js thresholds exactly — duplicated here only
// for display purposes (next-level progress bar), not as a source of truth.
...
function xpProgress(xp) {
  ...
  const next = LEVEL_THRESHOLDS[level]; // undefined at max level
  if (next === undefined) return { level, pct: 100, label: 'Max level reached' };
  const prev = LEVEL_THRESHOLDS[level - 1] || 0;
  const pct = Math.round(((xp - prev) / (next - prev)) * 100);
  return { level, pct, label: `${next - xp} XP to level ${level + 1}` };
}
      ...
      <motion.div style={S.box} initial={{ scale: 0.92, opacity: 0, y: 24 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.92, opacity: 0 }} transition={{ type: 'spring', stiffness: 200, damping: 22 }}>
        ...
        {error && (
          <div style={{ textAlign: 'center', padding: '2rem 0' }}>
            <p style={{ fontFamily: FONT.mono, fontSize: '0.48rem', color: '#EF5350' }}>{error}</p>
          </div>
        )}
        ...
        {data && (
          <>
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.1rem' }}>
              ...
              {uploadErr && <p style={{ fontFamily: FONT.mono, fontSize: '0.4rem', color: '#EF5350', marginTop: '0.3rem' }}>{uploadErr}</p>}
              ...
              <p style={{ fontFamily: FONT.mono, fontSize: '0.44rem', letterSpacing: '0.13em', color: COLOR.textMuted, marginTop: '0.2rem' }}>{data.user.email}</p>

              {/* XP / level — every point shown was earned for a real, verified action */}
              <div style={S.xpRow}>
                <span style={S.xpLevel}>LEVEL {progress.level}</span>
                <div style={S.xpBarTrack}><div style={{ ...S.xpBarFill, width: `${progress.pct}%` }} /></div>
                <span style={S.xpLabel}>{progress.label}</span>
              </div>
            </div>

            <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1.2rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button onClick={() => setTab('bookings')} style={{ ...S.tab, ...(tab === 'bookings' ? S.tabActive : {}) }}>BOOKINGS ({data.bookings.length})</button>
              <button onClick={() => setTab('ratings')} style={{ ...S.tab, ...(tab === 'ratings' ? S.tabActive : {}) }}>RATINGS ({data.ratings?.length || 0})</button>
              <button onClick={() => setTab('settings')} style={{ ...S.tab, ...(tab === 'settings' ? S.tabActive : {}) }}>SETTINGS</button>
            </div>

            {tab === 'bookings' && (
              <div style={S.list}>
                {data.bookings.length === 0 && <p style={S.empty}>No bookings yet — your booking history will appear here.</p>}
                {data.bookings.map(b => (
                  <div key={b._id} style={S.item}>
                    <div style={S.itemTitle}>{b.salonName}</div>
                    <div style={S.itemMeta}>{b.salonHub} · {b.service} · {b.date} · {b.timeSlot}</div>
            ...
            {tab === 'ratings' && (
              <div style={S.list}>
                {(!data.ratings || data.ratings.length === 0) && <p style={S.empty}>You haven't rated any salons yet.</p>}
                {data.ratings?.map(r => (
                  <div key={r._id} style={S.item}>
                    <div style={S.itemTitle}>
                      {r.salonId?.name || 'Salon'} · {r.salonId?.hub || ''}
            ...
            {tab === 'settings' && (
              <SettingsPanel token={token} onAvatarRemoved={reload} hasAvatar={!!data.user.avatarUrl} />
            )}
...
function SettingsPanel({ token, onAvatarRemoved, hasAvatar }) {
  ...
  return (
    <div style={S.list}>
      <div style={S.item}>
        <div style={S.itemTitle}>Change password</div>
        <input style={S.inp} type="password" placeholder="Current password" value={current} onChange={e => setCurrent(e.target.value)} />
        <input style={{ ...S.inp, marginTop: '0.5rem' }} type="password" placeholder="New password (8+ characters)" value={next} onChange={e => setNext(e.target.value)} />
        <button style={S.smallBtn} onClick={changePassword} disabled={busy || !current || next.length < 8}>{busy ? 'Updating…' : 'Update password'}</button>
        {msg && <p style={{ fontFamily: FONT.mono, fontSize: '0.4rem', color: msgErr ? '#EF5350' : '#4CAF50', marginTop: '0.4rem' }}>{msg}</p>}
      </div>
      {hasAvatar && (
        <div style={S.item}>
          <button style={S.smallBtnGhost} onClick={removeAvatar}>Remove profile photo</button>
        </div>
      )}
    </div>
  );
...
const S = {
  ...
  xpLevel: { fontFamily: FONT.mono, fontSize: '0.42rem', letterSpacing: '0.16em', color: COLOR.gold },
  ...
  xpLabel: { fontFamily: FONT.mono, fontSize: '0.38rem', color: COLOR.textGhost },
  tab: { padding: '0.4rem 0.8rem', background: 'transparent', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 20, color: COLOR.textMuted, fontFamily: FONT.mono, fontSize: '0.4rem', letterSpacing: '0.08em', cursor: 'pointer' },
  ...
  itemMeta: { fontFamily: FONT.mono, fontSize: '0.4rem', letterSpacing: '0.08em', color: COLOR.textGhost, marginTop: '0.25rem' },
  statusPip: { fontFamily: FONT.mono, fontSize: '0.38rem', letterSpacing: '0.1em', display: 'block', marginTop: '0.3rem' },
  comment: { fontFamily: FONT.body, fontSize: '0.78rem', color: COLOR.textMuted, marginTop: '0.4rem' },
  empty: { fontFamily: FONT.mono, fontSize: '0.44rem', color: COLOR.textGhost, textAlign: 'center', padding: '1.5rem 0' },
  ...
  smallBtn: { width: '100%', padding: '0.5rem', marginTop: '0.6rem', background: 'linear-gradient(135deg,#FFF2A8,#D4AF37)', border: 'none', borderRadius: 6, fontFamily: FONT.mono, fontSize: '0.42rem', letterSpacing: '0.1em', fontWeight: 700, color: '#000', cursor: 'pointer' },
  smallBtnGhost: { width: '100%', padding: '0.5rem', background: 'transparent', border: '1px solid rgba(239,83,80,0.3)', borderRadius: 6, fontFamily: FONT.mono, fontSize: '0.4rem', letterSpacing: '0.1em', color: '#EF5350', cursor: 'pointer' },
  deleteBtn: { width: '100%', padding: '0.6rem', background: 'transparent', border: '1px solid rgba(239,83,80,0.3)', borderRadius: 7, color: '#EF5350', fontFamily: FONT.mono, fontSize: '0.42rem', letterSpacing: '0.1em', cursor: 'pointer', marginTop: '0.5rem' },
};
