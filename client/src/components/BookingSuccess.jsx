  ...
  return (
    <motion.div style={{ position: 'fixed', inset: 0, zIndex: 950, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        ...
        <div className="booking-success-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.8rem' }}>
          {summaryGrid.map((item) => (
            ...
            >
              ...
              <div>
                <div style={{ fontFamily: FONT.mono, fontSize: '0.8rem', letterSpacing: '0.05em', color: COLOR.textGhost, marginBottom: '0.18rem' }}>{item.label}</div>
                <div style={{ fontFamily: FONT.body, fontSize: '1rem', color: COLOR.textPrimary, fontWeight: 300 }}>{item.val}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <motion.button 
          style={{ width: '100%', padding: '0.85rem', background: viaWhatsApp ? 'linear-gradient(135deg,#25D366,#1DA851)' : 'linear-gradient(135deg,#FFF2A8,#D4AF37)', border: 'none', borderRadius: 8, fontFamily: FONT.mono, fontSize: '0.8rem', letterSpacing: '0.1em', fontWeight: 700, color: viaWhatsApp ? '#fff' : '#000', cursor: 'pointer' }}
          onClick={onClose} 
