...
const SLOTS=['9:00 AM – 10:00 AM','10:00 AM – 11:00 AM','11:00 AM – 12:00 PM','12:00 PM – 1:00 PM','2:00 PM – 3:00 PM','3:00 PM – 4:00 PM','4:00 PM – 5:00 PM','5:00 PM – 6:00 PM','6:00 PM – 7:00 PM'];

// Normalizes a phone string to digits-only with country code for wa.me links.
// If the number has no country code, we assume India (+91) since this is a
// Hyderabad-only marketplace — but we never invent a number that isn't there.
function toWhatsAppNumber(phone) {
  if (!phone) return null;
  const digits = phone.replace(/[^\d]/g, '');
  if (!digits) return null;
  if (digits.length === 10) return `91${digits}`; // bare 10-digit Indian mobile
  return digits; // assume it already includes country code
}
...
export default function BookingModal({salon,onClose,onSuccess}) {
  ...
  // If we have a WhatsApp-capable number, default straight to that flow —
  // it's the most honest, most likely-to-work path. Otherwise fall back to
  // the in-app "logged request" form.
  ...
  // Real categories this salon is actually tagged with on OSM — if none,
  // offer the full category list since we genuinely don't know what they do.
  ...
  // Build the pre-filled WhatsApp message and open wa.me directly —
  // no backend round-trip needed, this just hands off to WhatsApp.
  const openWhatsApp = () => {
  ...
  const submitRequest=async()=>{
    setBusy(true);
    try {
      fetch(`${API}/api/users`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:form.email,name:form.name,phone:form.phone})}).catch(()=>{});
      const r=await fetch(`${API}/api/bookings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({salonId:salon._id,customerName:form.name,customerEmail:form.email,customerPhone:form.phone,service:form.category,date:form.date,timeSlot:form.slot,notes:form.notes,sourceHub:salon.hub})});
      const d=await r.json();
      if(!r.ok) throw new Error(d.error||'Request failed');
      onSuccess?.({salonName:salon.name,hub:salon.hub||'',date:form.date,slot:form.slot});
      onClose();
    } catch(e){
    ...
    <motion.div style={S.ov} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      ...
      <motion.div style={S.box} initial={{scale:0.92,opacity:0,y:24}} animate={{scale:1,opacity:1,y:0}} exit={{scale:0.92,opacity:0}} transition={{type:'spring',stiffness:200,damping:22}}>
        ...
        <div style={{textAlign:'center',marginBottom:'1.2rem'}}>
          ...
          <p style={{fontFamily:FONT.mono,fontSize:'0.47rem',letterSpacing:'0.17em',color:COLOR.textMuted,marginTop:'0.25rem'}}>{salon?.name||'Salon'} · {salon?.hub||'Hyderabad'}</p>
        </div>

        {/* Mode switch — if WhatsApp is available, offer it as primary, with
            a quiet link to the alternative for people who prefer it. */}
        {waNumber && (
        ...
        {/* WhatsApp mode — minimal form, then hand off to wa.me */}
        {mode==='whatsapp' && (
        ...
        {/* AURA request mode — full multi-step form, logged in our DB */}
        {mode==='request' && (
          <>
              ...
              {step==='confirm'&&(
                <motion.div key="conf" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
                  <div style={{background:'rgba(212,175,55,0.03)',border:'1px solid rgba(212,175,55,0.1)',borderRadius:8,padding:'1rem',marginBottom:'1rem'}}>
                    {[['👤','Name',form.name],['📧','Email',form.email],form.phone&&['📱','Phone',form.phone],['✂','Category',CATEGORY_LABELS[form.category]||form.category],['📅','Date',form.date],['⏰','Time',form.slot],form.notes&&['📝','Notes',form.notes]].filter(Boolean).map(([icon,label,val])=>(
                      <div key={label} style={{display:'flex',gap:'0.7rem',marginBottom:'0.5rem',alignItems:'flex-start'}}>
                        <span style={{fontSize:'0.9rem',flexShrink:0}}>{icon}</span>
                        <span style={{fontFamily:FONT.mono,fontSize:'0.4rem',letterSpacing:'0.1em',color:COLOR.textGhost,width:50,flexShrink:0}}>{label}</span>
