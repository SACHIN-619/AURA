// Immediate onboarding — shown when app opens
// Flow: pip → [allow location | search | skip]
// Never shows a dead screen — every path leads somewhere
...
export default function LocationOnboarding({onComplete}) {
  ...
  const { t } = useLanguage();
  const [stage,   setStage]   = useState('pip');  // pip | locating | search | denied
  ...
  const pickHub = async(hub) => {
    setBusy(true);
    try { await syncHub(hub); onComplete(); }
    catch { onComplete(); } // never block user
  ...
  const tryLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async pos => {
        const { hub, distanceKm, inServiceArea } = await resolveNearestHub(pos.coords.latitude, pos.coords.longitude);
        if(!inServiceArea){
          // Honest message — we don't silently teleport people in Mumbai into Jubilee Hills
          pushToast(
            distanceKm
              ? `You're ${distanceKm} km from Hyderabad. AURA currently covers Hyderabad only — showing nearest available hub.`
              : `Couldn't confirm you're near Hyderabad — showing nearest available hub.`,
            'info'
          );
        }
  ...
  const onSearch = val => {
    ...
    debRef.current = setTimeout(async()=>{
      try {
        // 1. Search our own DB first — real salon matches by name/hub/suburb
        const r = await fetch(`${API}/api/search/autocomplete?q=${encodeURIComponent(val.trim())}`);
        const d = await r.json();
        let combined = d.results || [];

        // 2. ALSO search OpenStreetMap for the raw area name itself, so a
        // neighborhood we haven't synced yet (e.g. "Kachiguda") still shows
        // up as a pickable destination — not capped to a fixed 8-hub list.
        try {
          const geo = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(val.trim()+', Hyderabad, Telangana, India')}&format=json&limit=4`, { headers:{ 'Accept-Language':'en' } });
        ...
        } catch { /* OSM search optional — DB results still work */ }

        setResults(combined);
      } catch {
        setResults(allHubs.filter(h=>h.hub.toLowerCase().includes(val.toLowerCase())).map(h=>({_id:h.hub,name:h.hub,hub:h.hub,count:h.count})));
      }
    },250);
  };
...
function SearchUI({t,query,results,allHubs,busy,onSearch,onPick,onHub}) {
  return (
    <>
      ...
      {!query && (
        <div>
          <p style={S.hubLabel}>{allHubs.length ? t('onboard_popular_areas') : 'LOADING AREAS…'}</p>
          <div style={S.hubGrid}>
            {/* Show every synced hub — no artificial cap. Grid wraps naturally. */}
            {allHubs.map(h=>(
              <button key={h.hub} style={S.hubChip} onClick={()=>onHub(h.hub)}>{h.hub}</button>
            ))}
          </div>
        </div>
      )}
