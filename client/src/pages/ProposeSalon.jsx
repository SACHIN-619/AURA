...
export default function ProposeSalon() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (images.length < 3) {
      return pushToast('At least 3 images are required for verification', 'error');
    }
    ...
    try {
      const payload = {
        name: formData.name,
        hub: formData.hub,
        address: formData.address, // simple string for address
  ...
  return (
    <div style={S.pageWrapper}>
      <div style={S.container}>
        <button onClick={() => navigate(-1)} style={S.backBtn}>← Back</button>
        <h1 style={{ fontFamily: FONT.display, color: COLOR.textPrimary, margin: '1rem 0 0.5rem 0' }}>List Your Shop</h1>
        <p style={{ fontFamily: FONT.body, color: COLOR.textMuted, marginBottom: '2rem' }}>
        ...
        <form onSubmit={handleSubmit} style={S.form}>
          <div style={S.grid}>
            ...
            <div style={S.card}>
              ...
              <textarea style={{...S.input, minHeight: '80px'}} name="description" value={formData.description} onChange={handleChange} placeholder="Tell us about your salon and services..." />
            </div>
