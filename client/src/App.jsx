...
function PageHeader() {
  const { activeHub } = useAura();
  return (
    <motion.div style={S.header} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
      <div>
        <h1 style={S.title}>Discover <span style={{ color: COLOR.gold }}>{activeHub || 'Hyderabad'}</span></h1>
...
const S = {
  ...
  sub: { fontFamily: FONT.mono, fontSize: '0.52rem', letterSpacing: '0.2em', color: COLOR.textMuted, marginTop: '0.4rem' },
